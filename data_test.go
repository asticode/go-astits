...
func TestIsPSIPayload(t *testing.T) {
	...
	esm.setUnlocked(uint16(256), uint16(1))
	assert.False(t, isPSIPayload(uint16(256), pm, esm))
	esm.setUnlocked(uint16(0x11), uint16(1))
	assert.False(t, isPSIPayload(uint16(0x11), pm, esm))
}
